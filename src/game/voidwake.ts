...
interface Options {
  ...
  commsRows: number;
  commsWrap: boolean;
  // 0.7.3 — when true, dock-time contract boards and new-game runs open the
  // mission-offer popup; when false, missions never auto-appear and the
  // player can enjoy pure sandbox flight.
  questOffers?: boolean;
  // 1.0.2.1 — point-defence mount behaviour: "auto" fires on the nearest
  // hostile, "target" prefers your tracked contact, "off" holds fire.
  turretMode?: "auto" | "target" | "off";
  // 1.0.3 — 3D output. `render3d` names a mode id from RENDER_3D_MODES
  // ("off" disables the whole pipeline); strength scales eye separation and
  // convergence is the world depth (in units) that sits on the screen plane.
  render3d?: string;
  render3dStrength?: number;     // 1..6
  render3dConvergence?: number;  // 500..8000 world units
}
